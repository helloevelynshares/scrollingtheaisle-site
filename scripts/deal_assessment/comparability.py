...
from weekly_ad_analysis.benchmarks import load_benchmark_thresholds

from .history_repository import load_family_meta, load_observation_series, resolve_feed_id
...
def _parse_oz_range(subtitle: str | None) -> tuple[float | None, float | None]:
    if not subtitle:
        return None, None
    # e.g. "regular size, 5–13 oz" or "5-13 oz"
    match = re.search(
        r"(\d+(?:\.\d+)?)\s*[–\-]\s*(\d+(?:\.\d+)?)\s*oz",
        subtitle,
        re.I,
    ...
    if meta:
        ...
        if lo is not None and hi is not None and offer_oz is not None:
            if offer_oz < lo - 0.05 or offer_oz > hi + 0.05:
                ...
                notes.append(
                    f"offer_oz={offer_oz} family_range={lo}-{hi}"
                )
            else:
