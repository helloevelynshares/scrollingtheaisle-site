...
Usage:
  ...
  python3 scripts/backfill_price_comparisons.py --product-id grapes
  python3 scripts/backfill_price_comparisons.py --product-ids grapes,eggs_18_count
  python3 scripts/backfill_price_comparisons.py --dry-run
...
SAFEWAY_BASELINES: dict[str, tuple[float, str]] = {
    ...
    "chobani_yogurt_tub": (7.99, "Chobani Non-Fat Plain Greek Yogurt 32 oz"),
    "chobani_yogurt_per_cup": (1.99, "Chobani Greek Yogurt 5.3 oz cup"),
...
VONS_BASELINES: dict[str, tuple[float, str]] = {
    ...
    "chobani_yogurt_tub": (7.99, "Chobani Greek Yogurt 32 oz"),
    "chobani_yogurt_per_cup": (1.99, "Chobani Greek Yogurt 5.3 oz cup"),
...
def load_existing_history() -> dict[str, dict[str, list]]:
    if not HISTORY_OUTPUT.is_file():
        return {}
    text = HISTORY_OUTPUT.read_text(encoding="utf-8")
    hist_match = re.search(
        r"export const COSTCO_PRICE_HISTORY.*?=\s*(\{.*?\});",
        text,
        re.S,
    )
    if not hist_match:
        return {}
    return json.loads(hist_match.group(1))


def load_existing_comparisons() -> dict[str, dict]:
    if not TS_OUTPUT.is_file():
        return {}
    text = TS_OUTPUT.read_text(encoding="utf-8")
    match = re.search(
        r"export const PRICE_COMPARISONS_BY_KEY.*?=\s*(\{.*?\});",
        text,
        re.S,
    )
    if not match:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Backfill grocery vs Costco comparisons.")
    parser.add_argument("--product-id", help="Single canonical product id")
    parser.add_argument("--product-ids", help="Comma-separated canonical product ids")
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print summary without writing SQL/TS",
    )
    return parser.parse_args()


def resolve_product_filter(args: argparse.Namespace) -> set[str] | None:
    if args.product_id:
        return {args.product_id.strip()}
    if args.product_ids:
        return {p.strip() for p in args.product_ids.split(",") if p.strip()}
    return None
...
def main() -> None:
    ...
    # Refresh local normalized cache from read-only costco-mvp data.
    from price_comparison.import_costco_data import import_costco_data  # noqa: E402

    import_costco_data()

    if product_filter:
        unknown = product_filter - set(CANONICAL_PACKAGES)
        if unknown:
            raise SystemExit(
                f"Unknown product id(s): {', '.join(sorted(unknown))}. "
                "Add metadata in scripts/price_comparison/canonical_metadata.py."
            )
    ...
    for slug in GROCERY_TRACKER_TO_COSTCO_REGION.values():
    ...
    for canonical_id, meta in CANONICAL_PACKAGES.items():
        if product_filter and canonical_id not in product_filter:
            continue
        for feed_id, feed_cfg in GROCERY_FEEDS.items():
            ...
            costco_item, match_note = match_costco_item(
                canonical_id, catalog, warehouse=slug,
            )
    ...
    if product_filter:
        history_by_product = load_existing_history()
        for canonical_id in product_filter:
            history_by_product.setdefault(canonical_id, {})
            for region in COSTCO_REGIONS:
                points = history_points_for_region(canonical_id, region, data_root)
                if points:
                    history_by_product[canonical_id][region] = points
    else:
        history_by_product = {}
        for canonical_id in CANONICAL_PACKAGES:
            history_by_product[canonical_id] = {}
            for region in COSTCO_REGIONS:
                points = history_points_for_region(canonical_id, region, data_root)
                if points:
                    history_by_product[canonical_id][region] = points
    ...
    if args.dry_run:
        print(
            f"[dry-run] comparisons: {len(results)} row(s): "
            f"inserted={inserted} updated={updated} (merge into existing TS)"
        )
        return
    ...
    if not product_filter:
        ...
        print(
            f"Incremental: {len(results)} comparison row(s): "
            f"inserted={inserted} updated={updated} (TS merge only; re-run full for SQL seed)"
        )

    history_body = json.dumps(history_by_product, indent=2)
...
""",
    ...
    print(f"Wrote {HISTORY_OUTPUT.relative_to(ROOT)}")
    print(f"Costco data root: {data_root}")
    for slug, catalog in catalogs.items():
        print(f"  {slug}: {len(catalog)} unique items from CSV merge")
    seattle_products = sum(
        1 for regions in history_by_product.values() if regions.get("seattle")
    )
    print(f"  seattle: parsed for {seattle_products} products (not wired to Safeway/Vons)")
    ...
    unmatched = list_unmatched_products(data_root)
    if unmatched:
        print(f"\nUnmatched in paired warehouse ({len(unmatched)}):")
