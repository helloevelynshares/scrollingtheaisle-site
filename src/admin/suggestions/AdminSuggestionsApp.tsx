...
export function AdminSuggestionsApp() {
  ...
  const loadSuggestions = useCallback(async () => {
    ...
    setError("");
    try {
      const data = await fetchAdminSuggestions();
      ...
      const text =
        loadError instanceof Error ? loadError.message : "Could not load suggestions";
      setError(text);
  ...
  async function runAction(
    ...
    setMessage("");
    try {
      await callAdminSuggestionActions(body);
    ...
    return (
      <main className="page-main admin-suggestions-main">
        <section className="admin-suggestions-panel">
          <h1>Review suggested tracker items</h1>
          <p className="admin-suggestions-lead">
            Sign in to review pending tracker suggestions.
          </p>
      ...
      <section className="admin-suggestions-panel">
        <header className="admin-suggestions-header">
          <div>
            <h1>Review suggested tracker items</h1>
            <p className="admin-suggestions-lead">
              Approve new items for the public voting list, reject duplicates, or merge
              into an existing approved item.
            </p>
          </div>
        ...
        {loading ? (
          <p className="admin-suggestions-loading">Loading pending suggestions…</p>
        ) : pending.length === 0 ? (
          <p className="admin-suggestions-empty">No pending suggestions right now.</p>
        ) : (
                      ...
                      >
                        ...
                        {approved.map((target) => (
                          <option key={target.id} value={target.id}>
                            {suggestionDisplayName(target)} ({target.vote_count} votes)
                          </option>
                        ))}
                      </select>
                    </label>
                    <button
                      ...
                      onClick={() =>
                        void runAction(
                          ...
                          `Merged “${item.raw_text}” into an approved item.`,
                        )
                      }
                    >
                      Merge (+1 vote)
