...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "list") {
      const [{ data: pending, error: pendingError }, { data: approved, error: approvedError }] =
        await Promise.all([
          supabase
            ...
            .select("id, raw_text, public_name, normalized_name, created_at")
            .eq("status", "pending")
            .order("created_at", { ascending: true }),
          supabase
            .from("tracker_vote_items")
            .select("id, public_name, raw_text, vote_count")
            .eq("status", "approved")
      ...
      return jsonResponse({
        pending: (pending ?? []) as PendingItem[],
        approved: (approved ?? []) as ApprovedItem[],
      });
    }
    ...
    if (action === "approve") {
      ...
      if (!existing || existing.status !== "pending") {
        return jsonResponse({ error: "Pending suggestion not found" }, 404);
      }
    ...
    if (action === "merge") {
      ...
      if (!targetItem || targetItem.status !== "approved") {
        return jsonResponse({ error: "Merge target must be an approved item" }, 400);
      }
    ...
    console.error("admin-suggestion-actions:", message);
